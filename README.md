...
`BIGINT UNSIGNED AUTO_INCREMENT` растёт последовательно. Вставки идут предсказуемо, ключ занимает 8 байт и даёт большой запас значений. Для учебного проекта хватило бы и `INT UNSIGNED`, но в реальных заказах и событиях счётчик может расти годами. Запас здесь дешевле, чем будущая миграция первичных ключей.
...
| `SET` | теги товара | Товар может иметь несколько простых флагов: `fragile`, `discount`, `digital`. |
